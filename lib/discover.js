  ...
  self.socket.on('message', (message, rinfo) => {
    try {
      ...
      if(msg.service == service) {
        ...
        isNew = !Object.keys(hosts).reduce(function (ips, domain) {
          return ips.concat(hosts[domain]);
        }, []).some(function (ip) {
          return ip === rinfo.address;
        });


        // Creates or overrides the host item
        hosts = msg.domains.reduce(function (hosts, domain) {
          if (!hosts[domain]) {
            hosts[domain] = [rinfo.address];
          }
          else if (hosts[domain].indexOf(rinfo.address) === -1) {
            hosts[domain].push(rinfo.address);
          }
          return hosts;
        }, hosts);
